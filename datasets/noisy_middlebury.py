...
from utils import make_coord, to_pixel_samples, visualize_2d, add_noise, seed_everything

class NoisyMiddleburyDataset(Dataset):
    def __init__(self, root, split='test', scale=8, augment=True, downsample='bicubic', pre_upsample=False, to_pixel=False, sample_q=None, input_size=None, noisy=True):
        super().__init__()
        self.root = root
        self.split = split
        self.scale = scale
        ...
        if self.split == 'train':
            ...
            self.depth_files = sorted(glob.glob(os.path.join(root, 'gt/*.png')))
...
if __name__ == '__main__':
    ...
    for method in ['bicubic']:
        for scale in [4, 8, 16]:
            ...
            d = NoisyMiddleburyDataset(root='./data/noisy_depth/middlebury', split='test', pre_upsample=True, augment=False, scale=scale, downsample=method, noisy=True)
            rmses = []
